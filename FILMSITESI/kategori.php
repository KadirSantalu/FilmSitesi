...
// Kategoriye ait toplam film sayısını al
$totalRecordsQuery = "SELECT COUNT(*) FROM film WHERE film_kategori = :category";
...
// Kategoriye ait filmleri listeleme
$filmQuery = "SELECT * FROM film WHERE film_kategori = :category LIMIT $startLimit, $limit";
$filmStmt = $db->prepare($filmQuery);
$filmStmt->bindParam(':category', $category, PDO::PARAM_STR);
$filmStmt->execute();

// Ust kısmını ekleyebilirsiniz.
?>
...
<div class="ana_govde">
    <div class="sol">
        <center><b><span class="font red"><?php echo $category; ?></span></b></center>
        <hr>

        <?php
        while ($row = $filmStmt->fetch(PDO::FETCH_ASSOC)) {
        ?>
            <div class="film">
                <a href="izle.php?id=<?php echo $row['film_id'] ?>">
                    <img class="a" src="FilmResimler/<?php echo $row['film_foto'] ?>">
                    <center>
                        <span class="span_baslik"><?php echo $row['film_baslik'] ?></span>
                    </center>
                </a>
                <hr>
                <table class="filmdet">
                    <tr>
                        ...
                        <td>Tür: <b><?php echo $row['film_kategori'] ?></b></td>
                    </tr>
                    <tr>
                        <td><img src="img/dil.png" height="15" width="17"></td>
                        <td>Dil: <b><?php echo $row['film_dil'] ?></b></td>
                    </tr>
                    <tr>
                        <td><img src="img/imdb.png" height="15" width="17"></td>
                        <td>IMDB: <b><?php echo $row['film_imdb'] ?></b></td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td colspan="2">
                            <center><a href="izle.php?id=<?php echo $row['film_id'] ?>"><button class="butbut">Filmi İzle</button></a></center>
                        </td>
                    </tr>
                </table>
            </div>
        ...
        <div class="pagination">
            ...
            if ($page > 1) {
                $newPage = $page - 1;
                echo '<a href="kategori.php?kategori=' . $category . '&page=' . $newPage . '" class="page prev">&laquo; Önceki</a>';
            } else {
            ...
            for ($i = $page - $record; $i <= $page + $record; $i++) {
                if ($i > 0 and $i <= $pageNumber) {
                    if ($i == $page) {
                        echo '<a href="kategori.php?kategori=' . $category . '&page=' . $i . '" class="page prev active">' . $i . '</a>';
                    } else {
                        echo '<a href="kategori.php?kategori=' . $category . '&page=' . $i . '" class="page prev">' . $i . '</a>';
                    }
                }
            }
            ...
            if ($page != $pageNumber) {
                $newPage = $page + 1;
                echo '<a href="kategori.php?kategori=' . $category . '&page=' . $newPage . '" class="page next">Sonraki &raquo;</a>';
            } else {
...
<?php
include "Alt.php"; // Alt kısmını ekleyebilirsiniz.
?>